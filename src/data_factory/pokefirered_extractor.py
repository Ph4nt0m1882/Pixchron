...
class PokefireredExtractor:
    def __init__(self, repo_path="pokefirered", out_dir="datasets_ready/pokefirered"):
        ...
        self.descriptions = {}
        desc_path = "desc_pokefirered.json"
        if os.path.exists(desc_path):
            with open(desc_path, "r", encoding="utf-8") as f:
                self.descriptions = json.load(f)
        
    def run(self):
        print("Parsing Pokefirered C Headers...")
        self.parse_anims()
    ...
    def parse_graphics(self):
        ...
        entries = re.finditer(r"const u16 (gObjectEventPic_[a-zA-Z0-9_]+)\[\]\s*=\s*INCBIN_U16\(\"(.*?\.4bpp)\"", content)
        for e in entries:
            var_name = e.group(1)
            bpp_path = e.group(2)
            png_path = bpp_path.replace('.4bpp', '.png')
    ...
    def process_image_augmentation(self, img):
        ...
        # Pokefirered background is always the top-left pixel (Palette 0)
        bg_color = pixels[0, 0]
    ...
    def extract_and_augment(self):
        ...
        for entity, info in self.entities.items():
            ...
            entity_base = entity.replace("gObjectEventGraphicsInfo_", "").lower()
            clean_base = entity_base.replace("normal", "")
            front_pic_path = os.path.join(self.repo_path, "graphics", "trainers", "front_pics", f"{clean_base}_front_pic.png")
...
if __name__ == "__main__":
    extractor = PokefireredExtractor()
    extractor.run()
